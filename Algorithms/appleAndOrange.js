...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
	const applesLandPoint = apples.map(el => el + a).filter(el => el >= s && el <= t).length;
	const orangesLandPoint = oranges.map(el => el + b).filter(el => el >= s && el <= t).length;

	console.log(applesLandPoint + '\n' + orangesLandPoint);
